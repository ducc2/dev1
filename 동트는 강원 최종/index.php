...
<ul id="accessibility-link">
	...
	<section id="mainVisual">
		...
		for($i=0;$i<sizeof($row);$i++){
			...
			if(mb_strlen($rows['ban_name'], "utf-8") > 18){
				$rows['ban_name'] = mb_substr($rows['ban_name'], 0,18, "utf-8"). '...';
			}

			/*$rows['ban_text'] = strip_tags($rows['ban_text']);
			if(mb_strlen($rows['ban_text'], "utf-8") > 200){
				$rows['ban_text'] = mb_substr($rows['ban_text'],0,200, "utf-8"). '...';
			}*/
						...
						<div class="child">
							<div class="childItem">
								...
								<a href="<?=$rows[ban_img_link]?>">
									<i><?=$rows['ban_name']?></i>
									<p><?=strip_tags($rows['ban_text'], '<br>')?></p>
...
$row			= $DB->dfetcharr($sql." LIMIT 0 , 12");
		...
		for($i=0;$i<$line_width;$i++){
			...
			$rows[subject]			= text_cut_kr($rows[subject],21);
