...
<div id="cont_right">
	...
	<div>
        ...
        <tr>
            <th scope="row"><label for="keyword">검색조건</label></th>
            <td>
				<select name="tot_state" id="tot_state" class="selectbox" onchange="this.form.submit()" style="width:120px;">
					<option value="">주문상태</option>
					<?=goods_order_state("", 2)?>
				</select>	
				<select name="payment_type" id="payment_type" class="selectbox" onchange="this.form.submit()" style="width:120px;">
					<option value="">결제방법</option>
					<?=get_payment_type("", 2)?>
				</select>	
				<script type="text/javascript">
					document.getElementById("tot_state").value	= "<?=$tot_state?>";
				...
				<input type="button" value=" 엑셀생성 " class="btn_excel" onclick="document.location.href='__excel_download.php?where=<?=$excel_whe?>&chart_type=<?=$chart_type?>&sdate=<?=$sdate?>&edate=<?=$edate?>&stype=statistic1'"><br>
                <span class="frm_info"><?=$help_img?> 검색조건을 이용해 상세 통계을 확인 할 수 있습니다.</span>
            </td>
        </tr>
	...
	<script type="text/javascript">
		...
		var data = google.visualization.arrayToDataTable([
				['<?=$chart_title?>', '합계', '무통장입금', '신용카드', '실시간계좌이체', '휴대폰결제', '가상계좌'],
				<?=$chart_data?>
		]);

		var options = {
			title: '매출 통계<?=$tot_state_str?><?=$payment_type_str?>',
			curveType: 'function',
			fontSize: 12,
			pointSize : 3,
	...
	<div>
		...
		<tr>
			...
			<th scope="col">실시간계좌이체</th>
			<th scope="col">휴대폰결제</th>
		...
		<?for($i=0;$i<sizeof($row);$i++){
			...
			<tr class="<?=$bg?>">
				...
				<td class="td_50_right"><?=$rows[sum_pay1]?></td>	
				<td class="td_50_right"><?=$rows[sum_pay2]?></td>	
				<td class="td_50_right"><?=$rows[sum_pay3]?></td>	
				<td class="td_50_right"><?=$rows[sum_pay4]?></td>	
				<td class="td_50_right"><?=$rows[sum_pay5]?></td>	
			</tr>
		<?	unset($rows);
			$idx--;
		}// 루프 끝?>
