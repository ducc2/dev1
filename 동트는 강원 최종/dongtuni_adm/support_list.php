...
<div id="cont_right">
	...
	<div class="area_search2">
		<form name="fsearch" id="fsearch" method="get">
			...
			<select name="sch_key" id="sch_key" class="selectbox" style="width:140px;">
				...
				<option value="a.phone">전화번호</option>
			</select>
	...
	<form name="flist" id="flist" action="./<?=$include_file?>_proc.php" onsubmit="return flist_submit(this);" method="post">
		...
		<tbody>
		<?if (sizeof($row)==0)  { ?>
		<tr><td colspan=8 align="center" height=150>등록된 자료가 없습니다.</td></tr>
		<? } ?>
		...
		<?for($i=0;$i<sizeof($row);$i++){
			...
			<tr class="<?=$bg?>">
				<td class="td_chk" >
					...
					<input type="hidden" name="nos[]" value="<?=$rows[no]?>">
				</td>
				...
				<td ><?=$rows[phone]?></td>
				<td ><?=$rows[content]?></td>
				<td><select name="extra_1" id="sch_key" class="selectbox" style="width:80px;" onchange="location.href='support_proc.php?state=extra_1&no=<?=$rows[no]?>&extra_1='+this.value">
				<option value="" <? if ($rows[extra_1]) { ?>selected<? } ?>>선택</option>
				<option value="활동" <? if ($rows[extra_1]=='활동') { ?>selected<? } ?>>활동</option>
				<option value="비활동" <? if ($rows[extra_1]=='비활동') { ?>selected<? } ?>>비활동</option>
				</select></td>
