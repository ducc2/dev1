...
<div id="cont_right">
	...
	<div>
        ...
        <tr>
            ...
            <td>
				...
				<input type="button" value=" 엑셀생성 " class="btn_excel" onclick="document.location.href='__excel_download_goods.php?where=<?=$excel_whe?>&chart_type=<?=$chart_type?>&sdate=<?=$sdate?>&edate=<?=$edate?>&stype=statistic1'"><br>
	...
	<script type="text/javascript">
		...
		var options = {
			title: '상품별 통계<?=$tot_state_str?><?=$payment_type_str?>',
			curveType: 'function',
			fontSize: 12,
			pointSize : 3,
	...
	<div>
		...
		<?for($i=0;$i<sizeof($row);$i++){
			...
			<tr class="<?=$bg?>">
				...
				<?if($chart_type=="week"){?>
					<td class="td_large20"><?=$goods_name?></td>	
...
function month_move(thisMonth, type){
	document.getElementById("chart_type").value		= "day";
