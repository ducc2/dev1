...
<div id="cont_right">
		...
		<?for($i=0;$i<sizeof($row);$i++){
			...
			<tr class="<?=$bg?>">
				<td class="td_chk" >
					...
					<input type="hidden" name="nos[]" value="<?=$rows[mem_no]?>">
				</td>
				...
				<td ><a href="<?=$PHP_SELF?>?mode=update&mem_no=<?=$mem_no?>&refer=<?=$refer?>">수정</a></td>
				<td ><a href="javascript:one_delete('<?=$mem_no?>')">삭제</a></td>
			</tr>
		<?
			$idx--;
		}// 루프 끝?>
	...
	function one_delete(no){
		...
		document.location.href = "<?=$PHP_SELF?>?mode=drop&mem_no="+no+"&refer=<?=$refer?>";
