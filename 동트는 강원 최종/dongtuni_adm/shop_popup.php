...
$upfilesname		= array("pop_img");
$updir				= DATA_PATH."popup/";
$thumb_dir			= DATA_PATH."popup/";
$upUrl				= "../data/popup/";
...
if(!$mode){
	...
	include_once("./shop_".$include_file."_list.php");

}elseif($mode=="form"){
	
	$state		= "insert";
	include_once("./shop_".$include_file."_form.php");
	...
	for($i=0; $i<count($upfilesname); $i++){
		if($row[$upfilesname[$i]]){
			...
			${$upfilesname[$i]."_tmp"}	= "<img src='$upUrl/$filename_real' width='230' height='80'><p>";
