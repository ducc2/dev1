...
if(!$mode){
	...
	if($_GET['yy'])	$where[] = "ban_yy ='".$_GET['yy']."'";
	...
	for($i=0; $i<count($upfilesname); $i++){
		if($row[$upfilesname[$i]]){
			...
			${$upfilesname[$i]."_tmp"}	= "<a href='./__download.php?path=../data/banner/".$filename_exp[1]."&name=".$filename_exp[0]."&board_id=".$board_id."&no=".$row_files[no]."'>$filename_real<p>";
