    ...
    <input type="hidden" name="cur_comment" value="<?=$cur_comment?>">
			<div class="comment">
				<fieldset class="write clearfix">
					...
					<textarea placeholder="내용을 입력하세요" name="content" class="textarea"></textarea>
    ...
    <input type="hidden" name="cur_comment" >
		...
		for($i=0;$i<sizeof($comment);$i++){
			...
			?>
						...
						<!--수정용 폼 -->
						<fieldset class="write clearfix edit" style="display:none" id="con_<?=$comments[no]?>">
								<!-- 로그인 하기전 -->
								<div class="login_inner clearfix">
								...
								<textarea id="inquiryText" placeholder="내용을 입력하세요" name="content_array[<?=$comments[no]?>]" class="textarea"><?=nl2br($comments[content])?></textarea>
								<a href="#none" onclick="editok('<?=$comments[no]?>');" title="등록"class="btn_write">수정</a>
						</fieldset>
