...
.con_wrap .container .contents ul.contents_list li a {display:block; margin:0 30px 30px 0; height:365px; border:1px solid #ddd;}
...
<!-- 스킨별 CSS 끝 ( 디자인 변경시 여기서 수정하시면 됩니다. ) -->
		...
		<div class="location_title"><? if($board_id == 'contents') {echo '컨텐츠관리';}else if($board_id == 'writer') {echo '여행작가 게시물';}else if($board_id == 'notice') {echo '이벤트/공지';}else if($board_id == 'news') {echo '뉴스레터';}else{echo '게시 컨텐츠';} ?><label for="checkAll" class="sound_only">전체 선택</label>
				<input type="checkbox" name="checkAll" value="1" id="checkAll"></div>
	...
	<div class="con_wrap clearfix">
		...
		<li>
				...
				<input type="hidden" name="nos[]" value="<?=$no?>"><?=$subject?></h4>
								<p><?=$rows[content]?></p>
							</a>
						</li>
		<?
		...
		<div class="list_bottom_page">
			...
			<input type="button" value=" 목 록 " id="btn_submit" class="button_white60x30" onclick="document.location.href='./board.php?board_id=<?=$board_id?>'"><input type="button" value=" 글쓰기 " id="btn_submit" class="button_white60x30" onclick="document.location.href='./board_write.php?board_id=<?=$board_id?>'">
			<?// 관리자만 노출
