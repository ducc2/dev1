...
//
// 게시물 삭제
}elseif($_POST["state"]=="comment_delete"){
...
exit;
	...
	$DB->query("DELETE FROM ".$table03." WHERE no = '".$_POST[cno]."'");

	js_alert_parent_reload("");

	js_alert_parent_href('', '/board/comment.php?board_id='.$_POST['board_id'].'&no='.$_POST['no']);
...
//페이지 이동
if($data[referer]){
	goto_url('/board/board.php?board_id='.$board_id);
}
if($referer){
	goto_url($referer);
}
