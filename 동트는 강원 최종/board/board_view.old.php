...
<!-- <iframe src="comment.php?board_id=<?=$board_id?>&no=<?=$no?>" id="ifr" name="ifr" onload="javascript:resizeIframe(this);" style="width:905px; height:160px" scrolling="no" border="no" allowTransparency="true" frameborder="0" framespacing="0" marginheight="0" marginwidth="0" ></iframe> -->
<? }?>
...
<!-- con_list-->
			<div class="con_list">
				<div class="list_tit">
					...
					<span class="list_all"><a href="/board/board.php?board_id=<?=$_GET['board_id']?>&cate=<?=$cate?>" >전체목록보기</a></span>
				</div>
				<ul>
<?
...
for($i=0;$i<sizeof($row_list);$i++){
			...
			?>
					<li class="clearfix">
						<a href="board_view.php?board_id=<?=$_GET['board_id']?>&no=<?=$rows_list['no']?>">
