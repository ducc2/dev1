			...
			<div class="hash_news">
				...
				<div class="news_list">
					...
					<? 					
					 ...
					 ?>
					<span class="hash_tag"><a href="/sub/tagArchives_list.html?board_id=<?=$board_id?>&tag=<?=urlencode(trim($temp[$i]))?>">#<?=$temp[$i]?></a></span>
					<?  } ?>
...
$row			= $DB->dfetcharr($sql." LIMIT 0 , 5");
		...
		for($i=0;$i<sizeof($row);$i++){ 
			...
			if($row_file[filename]){
				...
				$board_img	= "<img src='/comn/img/noImg.png'  alt='사진'>";
			}
			
			?>
							<li class="clearfix">
								<a href="../board/board_view.php?board_id=<?=$board_id?>&no=<?=$rows[no]?>&sch_key=<?=$sch_key?>&sch_text=<?=$sch_text?>&cate=<?=$rows[category]?>&refer=<?=$refer?>">
	...
	<? } ?>
					...
					<span class="hash_tag"><a href="/sub/tagArchives_list.html?board_id=<?=$board_id?>&tag=<?=urlencode(trim($rows[tag_content]));?>">#<?=$rows[tag_content]?></a></span>
					<?  } ?>
