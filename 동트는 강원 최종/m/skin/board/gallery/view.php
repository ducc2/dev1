...
?>
  ...
  <meta property="og:title" content="강원도 종합매거진 동트는 강원" />
  ...
  <meta property="og:image" content="<?=$share_img?>" />
	...
	<div class="locationwrap">
		<div class="location">
			<a href="/" class="icon_set icon_home"></a>
			<span class="home">&gt;</span>
			<a href="/board/board.php?board_id=<?=$_GET['board_id']?>&cate=<?=$cate?>" class="home"><?=$cate?></a>
		</div>
	</div><!--//locationwrap -->
		...
		<div class="container" id="print_view">
			<div class="contents" >
					...
					<div class="title clearfix">
						<h2><div class="tit_date"><?=$extra_2?><span>0<?=$extra_3?></span></div><?=$subject?></h2>
						<span class="sub_date"><?=substr($datetime, 0, 10)?></span>
						<span class="sub_date">View <?=$hit?></span>
						<button class="#none" onclick="copy_trackback('http://<?=$_SERVER['HTTP_HOST']?>/board/board_view.php?board_id=<?=$_GET[board_id]?>&no=<?=$_GET[no]?>')">http://<?=$_SERVER['HTTP_HOST']?>/board/board_view.php?board_id=<?=$_GET[board_id]?>&no=<?=$_GET[no]?><span class="add">주소복사</span></button>
					</div>
					...
					<div class="mapArea">
						...
						<div class="mapInfo">위치 : <?=$extra_5?><!-- 주소값 출력 --></div>
						<div id="map"></div>
						<script type="text/javascript" src="//apis.daum.net/maps/maps3.js?apikey=ef17df9a6fb652dc363c4fd4968a2b19&libraries=services"></script>
						<script>
							...
							geocoder.addr2coord('<?=$extra_5?>', function(status, result) { //주소값 출력
			...
			<div class="share_area clearfix">
				<div class="share_icon clearfix">
					...
					<a href="/board/board.php?board_id=<?=$_GET[board_id]?>&sch_key=&sch_text=&cate=<?=$cate?>"><span class="icon_set list" title="목록"></span></a>
					<!-- share alert-->
