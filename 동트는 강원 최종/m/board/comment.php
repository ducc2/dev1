...
<head>
	...
	<link rel="stylesheet" href="/m/comn/css/style.css">
	...
	<script src="/m/comn/js/jquery-1.11.2.min.js"></script>
	<script src="/m/comn/js/jquery-ui.min.js"></script>
	<script src="/m/comn/js/common.js"></script><!-- 메인페이지엔 없어야함 -->
	<script>
		var f_url = '<?=$loginUrl?>';
		...
		for($i=0;$i<sizeof($comment);$i++){
			...
			<!--수정용 폼 -->
						...
						<fieldset class="write clearfix" style="display:none" id="con_<?=$comments[no]?>">
								...
								<textarea id="inquiryText" placeholder="내용을 입력하세요" name="content_array[<?=$comments[no]?>]" style="width:95%"><?=nl2br($comments[content])?></textarea>
						...
						<li>
							...
							<div class="comment_view">
								...
								<span class="date"><?=substr($comments[datetime], 0, 10)?></span>
...
<? } ?>
			...
			<form name="flogin" ID="flogin" action="./comment_proc.php" method="POST">
		<input type="hidden" name="no" value="" id="noee">
		<input type="hidden" name="state" value="comment_delete">
		<div class="pop_pass">
			<div class="popBg"></div>
			<div class="popInput">비밀번호 : <input type="password" name="password" id="passs" style="width:80px;">
				<a href="#" class="btnMid" onclick="form_submit()">확인</a>
			</div>
			<a href="#close" class="btnclose">닫기</a>
		</div>
		</form>
		<script>
		$(document).ready(function(){
			...
			<? if ($this_page) {?>
				$('html,body').animate({scrollTop:$(".comment_list").offset().top},680); 
			<? } ?>	
		});
