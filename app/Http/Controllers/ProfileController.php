...
class ProfileController extends Controller
{
   ...
   //      return ( json_decode($req->pet_name));


   //      //배열인지 체크
   //      if (is_array($pet_name) ) {
   //          $pet_name = $req->pet_name; //펫 이름
   //          $pet_kind = $req->pet_kind; //펫 종류
   //          $pet_birthY = $req->pet_birthY; //펫 생일
   //          $pet_birthM = $req->pet_birthM; //펫 생일
   //          $pet_birthD = $req->pet_birthD; //펫 생일
   //          $pet_img = $req->pet_img; //펫 성별
   //          $pet_sex = $req->pet_sex; //펫 성별
   //          $pet_remark = $req->pet_remark; //펫 특징
   ...
   //                  "pet_name" => $pet_name[$i],
   //                  "pet_kind" => $pet_kind[$i],
   //                  "pet_birth" => $pet_birthY[$i] . "." .$pet_birthM[$i]  . "." . $pet_birthD[$i],
   //                  "pet_sex" => $pet_sex[$i],
   //                  "pet_remark" => $pet_remark[$i],
   ...
   //          }
   //          // if (!$pet_sex ) {
   //          //     return response()->json( array("err"=>false,"msg"=>"펫 성별을 입력해주세요." ) );
   //          // }

			// if ($pet_img!="undefined") {
			// 	$exMinmeType = explode("/",$pet_img->getMimeType()); //이미지인지 체크
			// 	if ($exMinmeType[0] != "image") { //이미지인지 체크
			// 		return response()->json( array("err"=>false,"msg"=>"이미지 파일이 아닙니다." ) );
			// 	}
			// 	$ext = $pet_img->getClientOriginalExtension(); //확장자
			// 	$fileName = "/pet/profile/" . md5("profile") . "_" . time() . "." . $ext; //사진

			// 	$fileUpload = $this->common->s3UploadPrc($fileName,$pet_img); //파일 업로드
			...
			// 				"created_date" => $created_date
			// 			);
			// 			$result = $this->common->add($tbl,$data);
			// 			break;
   ...
   //      }
   //      if ($result){
   //          return response()->json(array("err"=>true,"msg"=>"등록 성공" ));
   //      }
